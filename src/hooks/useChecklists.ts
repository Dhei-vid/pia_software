...
import { CreateChecklistRequest, UpdateChecklistRequest, Checklist } from "@/api/checklist/checklist-type";
...
export const useChecklists = () => {
  ...
  const createChecklist = useCallback(async (checklistData: CreateChecklistRequest) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateChecklist = useCallback(async (checklistId: string, checklistData: UpdateChecklistRequest) => {
    setLoading(true);
    setError(null);
    try {
      const response = await ChecklistService.updateChecklist(checklistId, checklistData);
      setChecklists(prev => 
        prev.map(checklist => 
  ...
  const fetchChecklists = useCallback(async (documentId?: string) => {
    ...
    try {
      const response = documentId 
        ? await ChecklistService.getChecklistsByDocument(documentId)
